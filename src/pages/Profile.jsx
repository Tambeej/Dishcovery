...
//               }
//             />
//             <span>Show recipes that contain my allergens</span>
//           </label>
//         </div>
...
function Profile() {
  ...
  // Ensure data loaded (AuthBridge also triggers on session changes)
  useEffect(() => {
    user.loadUserData();
  }, []);

  // keep local draft in sync when store updates
  useEffect(() => {
  ...
  async function save() {
    ...
    setStatus("");
    try {
      await user.savePreferences(prefs);
      setStatus(user.error ? "Save failed ❌" : "Saved ✅");
    } finally {
      setBusy(false);
      setTimeout(() => setStatus(""), 1500);
    }
  ...
  const greeting = useMemo(() => user.greeting, [user.greeting]);
  ...
  return (
    <div style={{ maxWidth: 720, margin: "40px auto", padding: 16 }}>
      ...
      {user.loading && (
        <div style={{ marginTop: 8, color: "#555" }}>Loading preferences…</div>
      )}
      {user.error && (
        <div style={{ marginTop: 8, color: "crimson" }}>{user.error}</div>
      )}
      ...
      >
        ...
        <div style={{ marginTop: 20, display: "flex", gap: 10 }}>
          <button onClick={save} disabled={busy || user.loading}>
            {busy ? "Saving…" : "Save preferences"}
          </button>
          {status && <span style={{ color: "#555" }}>{status}</span>}
        </div>
